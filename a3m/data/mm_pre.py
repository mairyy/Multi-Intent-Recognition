...
class MMDataset(Dataset):
    ...
    def __init__(self, label_ids, text_feats, video_feats, audio_feats, comet_feats, sbert_feats):
        ...
        f = [x.image_feature.numpy().tolist() for x in video_feats]
        self.video_feats = torch.tensor(f)
        video_ids = [[x.input_ids, x.input_attention_mask] for x in video_feats]
        self.video_ids = torch.tensor(video_ids)
    ...
    def __getitem__(self, index):
        ...
        sample = {
            ...
            'video_ids': self.video_ids[index],
