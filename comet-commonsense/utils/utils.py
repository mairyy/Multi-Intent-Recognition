...
def filter_beam_output(arr, inp):
    ...
    for i in range(0, 5):
        ...
        if (len(set(beam_w).intersection(set(words))) > 0 and sw[0] not in beam_w) or (sw[1] in arr[i] and sw[1] in inp ) or (sw[2] in arr[i]) or (sw[3] in arr[i]):
            continue
        if len(beam_w)==1:
            try:
        ...
        if len(getSentences(arr[i]))>=3:
            if additional!='' and nltk.pos_tag(nltk.word_tokenize(additional))[0][1] in ['VB','VBG','VBD']:
                additional = ''
